...
Section Limits.
  Lemma pp_ro_lim_prt {Γ} {e} {ϕ : sem_ro_ctx Γ -> Prop} {ψ} (f : sem_ro_ctx Γ -> R) :
  ...
  Lemma pp_ro_lim_tot {Γ} {e} {ϕ : sem_ro_ctx Γ -> Prop} {ψ} (f : sem_ro_ctx Γ -> R) :
    (INTEGER :: Γ) |-- [{fun x => ϕ (snd x)}] e  [{y : REAL | fun x => Rabs (y - f(snd x)) < pow2 (- fst x)}] ->
    ((fun y (x : sem_ro_ctx Γ) => ϕ x /\ y = f x) ->>> ψ) ->
    Γ |-- [{ϕ}] Lim e [{y : REAL | ψ y}].
  Proof.
...
Section Commands.
  ...
  Lemma pp_rw_new_var_prt {Γ Δ} {e c} {τ σ}
  ...
  Lemma pp_rw_assign_prt {Γ Δ} {k} {e} {τ} {ϕ} {ϕ'} {ψ : post} {θ} 
    (a : assignable Δ τ k) :
    (Δ ++ Γ) |-- {{ϕ'}} e {{y : τ | θ y}} ->
    ((fun x  => ϕ (tedious_sem_app Δ Γ x)) ->> ϕ') ->
