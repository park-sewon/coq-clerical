...
Definition sem_ro_prt {Γ} {e} {τ} {wty} (t : ro_prt wty) :=
  ...
  forall γ, P γ ->
            ...
            pdom_nempty V /\ forall v, pdom_char V v -> forall v', v = total v' -> Q v' γ.
...
Definition sem_ro_tot {Γ} {e} {τ} {wty} (t : ro_tot wty) :=
  ...
  forall γ, P γ ->
            let V := sem_ro_comp Γ e τ wty γ in
            pdom_nempty V /\ forall v, pdom_char V v -> exists v', v = total v' /\ Q v' γ.
...
Definition sem_rw_prt {Γ Δ} {c} {τ} {wty} (t : rw_prt wty) :=
  ...
  forall γ δ, P (δ, γ) ->
            ...
            pdom_nempty V /\ forall v, pdom_char V (total v) -> Q (snd v) (fst v, γ).
...
Definition sem_rw_tot {Γ Δ} {c} {τ} {wty} (t : rw_tot wty) :=
  ...
  forall γ δ, P (δ, γ) ->
              let V := sem_rw_comp Γ Δ c τ wty γ δ in
              pdom_nempty V /\ exists v, pdom_char V (total v) -> Q (snd v) (fst v, γ).
